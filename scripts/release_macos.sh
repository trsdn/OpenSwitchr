...
ASSET_BASE="$DIST_DIR/OpenSwitchr-v${version}-macOS-arm64"
DMG_PATH="$ASSET_BASE.dmg"
APP_PATH="${APP_PATH:-$PROJECT_DIR/.build/release/OpenSwitchr.app}"
ZIP_PATH="$ASSET_BASE.zip"
CAN_NOTARIZE=false

# Notarization needs Apple credentials, so a release built without them still
# produces a signed, verifiable DMG rather than failing outright. Gatekeeper
# will warn on first launch; that is the honest tradeoff, not a silent one.
