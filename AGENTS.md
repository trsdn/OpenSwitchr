...
- **Add a secret, an environment, or a write permission to any workflow here,
  except the one documented exception below.** Every workflow in
  `.github/workflows` runs with `contents: read` and no secrets, and that is the
  property that makes this repository safe to build from. A feature that needs
  write access to the repository is not worth it.
  **The exception is `stats.yml`, and only it.** The quality standard's `P09`
  wants the repository activity card generated by a workflow and committed, which
  needs `contents: write`. That workflow is allowed it under three limits: it
...
use a secret or an environment. The only workflow with write permission is
`stats.yml`, limited to the generated `repo-stats` branch (see "Never, under any
circumstances").
